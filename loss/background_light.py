...
class BackgroundLightSim:
    ...
    def run_sim(self) -> float:
        ...
        I = lambda v, T: float((2*h*v**3) / (c**2 * math.exp((h*v)/(k*T)) - 1))
    ...
    #     # Omega = solid angle from which telescope can be seen from earth
    #     Omega = math.pi * (self.sat_telescope_diameter /2)**2 / self.sat_altitude**2
    #     N_Sun = e * a_Earth * N_Moon * (Lamda/d_EM**2) * Omega
