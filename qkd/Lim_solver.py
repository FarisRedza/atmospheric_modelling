...
def brute_search_parallel_equality(m, delta, eps_qkd, t, f):
    ...
    beta_range = np.linspace(0,0.5,200)
    nu_range = np.linspace(0,0.5-delta,200)
    xi_range = np.linspace(0,0.5-delta,200)
...
def smart_optimise(m, delta, eps_qkd, t, f):
    init_vals = brute_search_parallel_equality(m, delta, eps_qkd, t, f)[1:]
    constraints = [
    ...
    if -result.fun>=0:
        return  -result.fun
    return objective(init_vals, delta, m, eps_qkd, t, f)